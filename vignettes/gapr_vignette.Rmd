...
Five functions are supplied:
  ...
  2. `summarize_gapr()` : for summarizing the gapminder data,
...
all countries with a lifeExp less than 50 year are grouped and returned.

```{r}
dat <- load_gapr()
# !!!
# this function somehow works when this block is executed but shows error 
# "not exported by gapr", when knitting, even thought in the NAMESPACE file 
...
\begin{align*}
y_i &= \beta_0 + \beta_1 x_1 + \b_1 z_1 + \b_2 z_2  + \epsilon_i\\
\epsilon_i & \sim \mbox{N}\left(0, \sigma^2\right)
\end{align*}
...
The `plot()` function can also be used to plot the output the fitted model. Taking `randmod` from above, a plot can be created by typing `plot(randmod)`.
